async function getProjects() {
    ...
    // Next we call the fetch function and pass in the url and the method. Themethod is set to `GET` because we are fetching data. Fetch returns a"promise".
...
// ...on the other hand, if the request was successful then we will returnthe data from the response.// Turing the response to JSON takes time so we need to use the `await`keyword again.
